...
export const api = {
  ...
  getTournamentGroupAdvancement: (opts?: { iterations?: number; seed?: number }) =>
    getJson<TournamentGroupAdvancementOut>(
      `/api/tournament/group-advancement?iterations=${opts?.iterations ?? 1000}&seed=${opts?.seed ?? 0}`,
    ),
