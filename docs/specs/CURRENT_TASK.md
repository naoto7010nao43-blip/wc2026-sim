...
**Smaller, lower-severity follow-up flagged but not addressed:** `players.json` (read by 13 diagnostics/benchmark scripts) could still drift from `players2026_official.json`/`playerRatings2026_estimated.json` (the v2 player layer) over time, the same way `teams.json` did -- but checked first, and unlike teams, no apply-script currently writes player corrections to `players.json` (`apply_fifa_squad_field_updates.py` already targets `players2026_official.json` directly), so this can only cause wrong diagnostics, not wrong production data, today. Recommended as a smaller follow-up for Codex, alongside Ghana's overtaken fifa_rank candidate (Tunisia's manager-identity item is resolved, see the Handoff Notes section below).
